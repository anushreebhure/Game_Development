...
instruction_background = pygame.image.load("bg-3.png")
pause_bg = pygame.image.load("bg-2.jpg")
car_width = 56
pause = False
    ...
    while introduction:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                quit()
                sys.exit()
        gamedisplays.blit(instruction_background, (0, 0)) #for bg after clicking on instruction
        largetext = pygame.font.Font('freesansbold.ttf', 50)
        smalltext = pygame.font.Font('freesansbold.ttf', 20)
        mediumtext = pygame.font.Font('freesansbold.ttf', 30)
        extrasmalltext = pygame.font.Font('freesansbold.ttf', 15)
        TextSurf, TextRect = text_objects("INSTRUCTIONS", largetext)
        TextRect.center = ((405), (100))
        # textSurf, textRect = text_objects("This is a Simple Car Game in which you need dodge from incoming cars. If you Dodged 7 cars you will reach Level1 where speed of incoming cars will get increased by 2 units, similarly if you move to further levels incoming cars speed will get increased and possibility of crashing will also increased.", extrasmalltext)
        # textRect.center = ((375), (180))
        textSurf, textRect = text_objects("This is a Simple Car Game in which you need dodge from incoming cars.",
            smalltext)
        textRect.center = ((375), (180))
    ...
    while pause:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                quit()
                sys.exit()
        gamedisplays.blit(pause_bg, (0, 0))
        largetext = pygame.font.Font('freesansbold.ttf', 50)
...
def countdown_background():
    # this will create a new font object from the system fonts(predefined fonts in system)
    # None means default font is set, size is 100
    # create an object for
    # pygame provides no way for directly drawing text on the existing surface
    # for adding the text on the surface we use render() method inside the pygame.font module
    # render method will create a image of the text or the surface the text(a text image)
    # then using blit method we can display the text as image
...
def text_objects1(text,font):
    textsurface1=font.render(text,True,Bright_Red)
...
def game_loop():
    ...
    while not bumped:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_b:
                    obstacle_speed -= 2
            # if key is released(or else car will vanish)
        ...
        x += x_change
        pause = True
        gamedisplays.fill(gray)#background color
        ...
        car(x, y)#x is responsible for movement of the car in terms of x coordinate
        #the game should get over if the car touches the white lane
        #car1.jpg has dimensions 56*125(width*height)
    ...
    # enemy car should appear from top of the screen as soon as they go beyond the screen(they should start coming from top again)
        if obs_starty > display_height:
            ...
            passed = passed + 1
            score = passed * 10
            # increment level of the game and show up on the screen
            if int(passed) % 10 == 0:
                level = level + 1
                obstacle_speed + 2
                largetext = pygame.font.Font("freesansbold.ttf", 40)
                textsurf, textrect = text_objects("WOW Going GOOD! LEVEL" + str(level), largetext)
                textrect.center = ((display_width / 2), (display_height / 2))
                gamedisplays.blit(textsurf, textrect)
                pygame.display.update()
                time.sleep(3)
        # if enemy crosses y coord of our car then clash
        if y < obs_starty + obs_height:
            # right side-->x>obs_startx ; x+car_width>obs_startx
            # left side-->obs_startx + enemy_width ; x+car_width
            if x > obs_startx and x < obs_startx + obs_width or x + car_width > obs_startx and x + car_width < obs_startx + obs_width:
                crash()
        button("PAUSE", 650, 0, 150, 50, blue, bright_blue, "pause")
        pygame.display.update()
        clock.tick(60)# to slower down the movement of the car(slower down functionality)
        #it will never run more than 100 frames per second
